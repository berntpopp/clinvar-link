...
OWN loggers, BEFORE this repo's :func:`install_validation_error_handler` / the Layer-3
:func:`install_protocol_error_handler` backstop reshape the caller-facing frame. These
records reflect caller input into a log/telemetry sink independent of who the caller
is. Examples (all real, this FastMCP/mcp version — confirmed by the not-found guard
probe against pristine main):
...
* ROOT / ``mcp.shared.session`` (bare ``logging.warning``) — ``Failed to validate
  request: <pydantic error with the raw malformed URI>`` / ``Message that failed
  validation: …``

``mask_error_details=True`` masks the tool *response*, not these *log* records, and the
prior error-sanitation sweep's ``_ValidationLogFilter`` only drops the arg-validation
record on ``fastmcp.server.server``. This filter neutralizes the full reflection set at
the SOURCE logger (a logging filter only runs for records emitted on the logger it is
attached to — ancestor filters are skipped during propagation), replacing the whole
message and clearing ``args``/``exc_info`` so raw caller input never lands in a log sink
at ANY level (the fleet PII / log-hygiene invariant), then a WARNING+ prefix fallback
catches any other framework record whose interpolated args carry caller-derived detail.
"""
...
#: each (root covers ``mcp.shared.session``'s bare ``logging.warning``; the explicit
#: ``mcp.shared.session`` entry is belt-and-braces should it ever log on its own named
#: logger); ``fastmcp`` / ``mcp`` are kept for the WARNING+ fallback and their
#: non-propagating Rich handlers.
_SOURCE_LOGGERS = (
