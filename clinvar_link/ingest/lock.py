...
funnel through ``clinvar-link-data``/``refresh``. This lock ensures only one of
them downloads + rebuilds at a time, so they never clobber each other's temp
file.
...
from clinvar_link.exceptions import DataUnavailableError
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    raise DataUnavailableError(
                        f"Timed out after {timeout}s waiting for the ClinVar build lock; "
